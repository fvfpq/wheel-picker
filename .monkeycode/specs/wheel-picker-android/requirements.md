...
一个安卓转盘选择 App（Wheel Picker）。用户可自由编辑转盘选项（文字、颜色、权重），点击旋转后随机选出结果；活动主办方可通过隐藏后台指定下一次转盘停在哪一个选项，用于活动抽奖等场景的结果控制。
...
- **正常模式**: 未启用后台指定时，结果由权重随机决定
- **后台控制模式**: 通过隐藏入口进入，可指定下一次旋转结果
...
### Requirement 7: 隐藏后台控制

**User Story:** AS 活动主办方, I want 通过隐藏后台指定下一轮结果, so that 在需要时可控地决定抽奖结果。

#### Acceptance Criteria

1. WHEN 用户连续快速点击转盘页面顶部区域 5 次，系统 SHALL 弹出密码校验对话框
2. WHEN 用户输入正确后台密码，系统 SHALL 进入后台控制模式
3. WHEN 用户在后台中指定某个选项，系统 SHALL 使下一次旋转停止在该选项扇区
4. WHILE 后台指定存在，系统 SHALL 在转盘页面以隐蔽方式标记该指定状态
5. WHEN 后台指定的旋转完成，系统 SHALL 清除该次指定状态，恢复随机模式
6. WHEN 用户输入错误密码，系统 SHALL 拒绝进入并保留在正常模式
7. 后台控制模式入口 SHALL 需要密码保护，防止普通用户误入

### Requirement 8: 历史记录

**User Story:** AS 活动主办方, I want 查看历史旋转结果, so that 事后核对抽奖过程与公平性。
